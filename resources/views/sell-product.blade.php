...
<head>
    <meta charset="utf-8">
    <title>Sell Product - Store Keeper</title>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="bg-secondary rounded h-100 p-4">
                            ...
                            <form action="/sell/@foreach($data as $item){{ $item->id }}  @endforeach" method="POST">
                                @csrf
                                <div class="mb-3">
                                    <label for="" class="form-label">Name</label>
                                    <input type="text" class="form-control" name="name" placeholder="Product Name" value="@foreach($data as $item){{ $item->name }}  @endforeach" readonly>
                                </div>
                                <div class="mb-3">
                                    <label for="" class="form-label">Price</label>
                                    <input type="text" class="form-control" name="price" placeholder="Price" value="@foreach($data as $item){{ $item->price }}  @endforeach" readonly>
                                </div>
                                <input type="text" name="id" value="@foreach($data as $item){{ $item->id }}  @endforeach" hidden>
                                <input type="text" name="qty" value="@foreach($data as $item){{ $item->qty }}  @endforeach" hidden>
                                <div class="mb-3">
                                    <label for="" class="form-label">Quantity</label><br>
                                    <span class="text-success">Total Quantity=@foreach($data as $item){{ $item->qty }}  @endforeach</span>
